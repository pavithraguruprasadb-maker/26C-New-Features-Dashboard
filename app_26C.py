...
# ─────────────────────────────────────────────
    ...
    st.markdown('<div style="background-color:#fffbeb; padding:8px 14px; border-radius:6px; border-left:4px solid #f59e0b; margin-bottom:8px; font-size:0.82rem; color:#444;">📌 <b>Note:</b> The New Features tab shows features marked Training Required = Yes, TBD, or left blank. Features marked "No" are not shown (no video will be made for them). The Unboxing tab shows <b>every</b> unboxing video.</div>', unsafe_allow_html=True)
    ...
    # NF tab: Yes + TBD + Not Set (Training = No stays excluded — no video will be made)
    ...
    def style_r9_detail(row):
        ...
        return styles
    # ── Every feature is counted in exactly ONE column, so all columns add up to Total ──
    R9_STATUS_ORDER = ['Awaiting CDL Date','Development WIP','In Review (A)','In Review Fixes (A)','In Post Production (A)','In QA (A)','In QA Fixes (A)','In Setup (A)','Released (A)']
    def build_pivot(df_src):
        d = df_src.copy()
        outc  = d.get('Issue Resolution Outcome', pd.Series('', index=d.index)).fillna('').astype(str).str.strip().str.lower()
        issue = d.get('Type of Issue', pd.Series('', index=d.index)).fillna('').astype(str).str.strip()
        def category(i):
            s = d.at[i, 'Final Overall Status']; t = d.at[i, 'Training Required? ']
            if s == 'Released (A)': return 'Released (A)'
            if s == 'Feature Dropped' or outc[i].startswith('dropped'): return 'Feature Dropped'
            if s != 'Not Set': return s                                     # any in-progress status
            # from here on, the status is blank:
            if t == 'Not Set': return 'Training Required field Not set'
            if t == 'No': return 'No Video (Training = No)'                 # appears on Unboxing tab only
            if issue[i] != '': return 'NF Issues'
            return 'Pending'                                                # fits nowhere = tracker not updated
        d['__cat'] = [category(i) for i in d.index]
        pivot = d.groupby(['Pillar', '__cat']).size().unstack(fill_value=0)
        tail = ['Feature Dropped', 'Training Required field Not set', 'NF Issues', 'No Video (Training = No)', 'Pending']
        for c in R9_STATUS_ORDER + tail:
            if c not in pivot.columns: pivot[c] = 0
        extra = [c for c in pivot.columns if c not in R9_STATUS_ORDER + tail]  # unexpected status values stay visible
        pivot = pivot[R9_STATUS_ORDER + extra + tail]
        pivot['Total'] = pivot.sum(axis=1)
        if pivot['No Video (Training = No)'].sum() == 0:
            pivot = pivot.drop(columns=['No Video (Training = No)'])
        # Info only (not added into Total): training decision missing, but work already has a status
        tns = d[(d['Training Required? '] == 'Not Set') & (d['Final Overall Status'] != 'Not Set')]
        pivot['Training Not Set (Status Present)'] = tns.groupby('Pillar').size().reindex(pivot.index).fillna(0).astype(int)
        if pivot['Training Not Set (Status Present)'].sum() == 0:
            pivot = pivot.drop(columns=['Training Not Set (Status Present)'])
        # Training counts per pillar (Yes / TBD / No). "No" auto-hides when empty.
        tr = d.groupby(['Pillar', 'Training Required? ']).size().unstack(fill_value=0)
        for label in ['Yes', 'TBD', 'No']:
            pivot[f'Training = {label}'] = (tr[label].reindex(pivot.index).fillna(0).astype(int)
                                            if label in tr.columns else 0)
        if pivot['Training = No'].sum() == 0:
            pivot = pivot.drop(columns=['Training = No'])
        pivot = pivot.reset_index()
        tr_cols  = [c for c in ['Training = Yes', 'Training = TBD', 'Training = No'] if c in pivot.columns]
        end_cols = [c for c in ['Feature Dropped', 'Training Required field Not set', 'Training Not Set (Status Present)', 'NF Issues', 'No Video (Training = No)'] if c in pivot.columns]
        ordered = ['Pillar'] + tr_cols + R9_STATUS_ORDER + extra + end_cols + ['Total', 'Pending']
    ...
    with tab1:
        ...
        st.markdown('''<div style="background-color:#f8fafc; padding:10px 14px; border-radius:6px; border-left:4px solid #64748b; margin-top:4px; margin-bottom:8px; font-size:0.8rem; color:#444; line-height:1.9;">
<b>How to read this table</b> — every feature is counted in exactly one column, so all columns add up to Total.<br>
🟢 <b>Released (A)</b> — video is published and done.<br>
🔵 <b>Awaiting CDL Date → In Setup</b> — video is being worked on; the column shows which stage it has reached.<br>
🔴 <b>Feature Dropped</b> — no video will be made. Either the status says "Feature Dropped", or the PM team's answer to a reported problem was "Dropped" (bug, heavy setup, environment issue, or moved to 26D).<br>
🟠 <b>Training Required field Not set</b> — no one has decided yet whether this feature needs a video (the Training Required box is empty, and no work has started).<br>
🟣 <b>NF Issues</b> — the CDL hit a problem, reported it, and is waiting for the PM team's answer. Work is on hold, not forgotten.<br>
⚠️ <b>Pending</b> — fits none of the above: no progress, no problem reported, no decision. <b>This should always be 0.</b> Any number here means the tracker needs updating.
</div>''', unsafe_allow_html=True)
        st.markdown("---")
    ...
    with tab2:
        if len(df_r9_ub) == 0:
            ...
            st.markdown('''<div style="background-color:#f8fafc; padding:10px 14px; border-radius:6px; border-left:4px solid #64748b; margin-top:4px; margin-bottom:8px; font-size:0.8rem; color:#444; line-height:1.9;">
<b>How to read this table</b> — every video is counted in exactly one column, so all columns add up to Total.<br>
🟢 <b>Released (A)</b> — video published. &nbsp;🔵 <b>Other status columns</b> — video in progress at that stage. &nbsp;🔴 <b>Feature Dropped</b> — video will not be made.<br>
⚪ <b>No Video (Training = No)</b> — marked as not needing a video; shown here just for completeness.<br>
🟡 <b>Training Not Set (Status Present)</b> — work has started but the Training Required box was never filled in. Info only (already counted in its status column) — please get the box filled.<br>
⚠️ <b>Pending</b> — fits nowhere: no progress, no problem, no decision. <b>Should always be 0.</b>
</div>''', unsafe_allow_html=True)
            st.markdown("---")
...
# ─────────────────────────────────────────────
    ...
    # ══════════════════════════════════════════════════════════════════
    # RAC CALENDAR — 26C NF days allotted per CDL
    # Reads the sprint RAC workbook (tabs = pillars, 5-row Mon-Fri blocks
    # per CDL, columns = weeks). Counts every cell containing "26C NF"
    # from 06-Jul onward; multi-task cells count as fractions; struck-
    ...
    @st.cache_data
    def compute_rac_nf_days(path, as_of):
        """One row per CDL: exact 26C NF days, first/last 26C NF date, and
        days elapsed (26C NF cells fully in the past as of `as_of`).
        `as_of` is a YYYY-MM-DD string so the daily cache refreshes."""
        ...
        def _live_text(cell):
            ...
            if v is None:
                return '', ''
            if isinstance(v, _CRT):                  # mixed formatting: run strike wins
                full, live = '', ''
                for part in v:
                    text = getattr(part, 'text', str(part))
                    struck = bool(getattr(getattr(part, 'font', None), 'strike', False))
                    full += text
                    if not struck:
                        live += text
                return full, live
            full = str(v)                            # plain cell: cell font strike wins
            live = '' if bool(getattr(cell.font, 'strike', False)) else full
            return full, live
        ...
        def _split(txt):
            ...
            for p in _re.split(r'[\n\r]+', txt):
                for q in _re.split(r'\s+/\s+', p):   # ' / ' separates tasks; 'Dev/Rec' doesn't
                    q = q.strip()
                    if q:
                        out.append(q)
            return out
        ...
        def _frac(live):
            ...
            if n == 0:
                return 0.0
            nf = [t for t in tasks if '26c nf' in t.lower()]
            if not nf:
                return 0.0
            if any('half day' in t.lower() for t in nf):
                return 0.5
            return min(len(nf) / n, 1.0)

        try:
            wbk = _lwb(path, rich_text=True, data_only=True)
        except FileNotFoundError:
            return pd.DataFrame(columns=['CDL', 'RAC Start', 'RAC End', 'RAC NF Days'])

        recs = []
        for s in RAC_SHEETS:
            if s not in wbk.sheetnames:
                continue
            ws = wbk[s]
            week_starts = {}
            for c in range(2, ws.max_column + 1):
                d = pd.to_datetime(ws.cell(row=1, column=c).value, errors='coerce')
                if pd.notna(d):
                    week_starts[c] = d
            blocks = []
            for r in range(3, ws.max_row + 1):
                v = ws.cell(row=r, column=1).value
                name = v.as_list()[0] if isinstance(v, _CRT) else v
                if name is not None and str(name).strip():
                    blocks.append((r, str(name).strip()))
            for bi, (r0, cdl) in enumerate(blocks):
                r1 = blocks[bi + 1][0] if bi + 1 < len(blocks) else ws.max_row + 1
                for off in range(min(r1 - r0, 5)):          # Mon..Fri
                    for c, wstart in week_starts.items():
                        full, live = _live_text(ws.cell(row=r0 + off, column=c))
                        if not full.strip():
                            continue
                        day = wstart + pd.Timedelta(days=off)
                        if day < RAC_START:
                            continue
                        f = _frac(live)
                        if f > 0:
                            recs.append({'CDL': cdl, 'Date': day, 'Frac': f})
        if not recs:
            return pd.DataFrame(columns=['CDL', 'RAC Start', 'RAC End', 'RAC NF Days', 'RAC Elapsed'])
        d = pd.DataFrame(recs)
        out = d.groupby('CDL').agg(**{
            'RAC Start':   ('Date', 'min'),
            'RAC End':     ('Date', 'max'),
            'RAC NF Days': ('Frac', 'sum'),
        }).reset_index()
        # elapsed = NF days fully in the past (Date < as_of); today's cell not yet counted
        elapsed = d[d['Date'] < as_of_ts].groupby('CDL')['Frac'].sum().rename('RAC Elapsed')
        out = out.merge(elapsed, on='CDL', how='left')
        out['RAC Elapsed'] = out['RAC Elapsed'].fillna(0.0).round(2)
        out['RAC NF Days'] = out['RAC NF Days'].round(2)
        return out

    # single computation, reused for the in-loop RAC status and the merged columns
    rac_days_df = compute_rac_nf_days(RAC_FILE_PATH, today_r14.strftime('%Y-%m-%d'))
    ...
    for cdl in all_cdls:
        ...
        # ══════════════════════════════════════════════════════════════
        # RAC-BASED PACE — parallel to the SharePoint-based columns above.
        # Elapsed days come from the RAC calendar (26C NF cells fully in
        # the past, fractions included), so a CDL is only expected to
        # deliver 3/day for days they were actually ALLOTTED to 26C NF.
        # ══════════════════════════════════════════════════════════════
        ...
        if rac_elapsed is None:
            ...
            if total_count > 0 and actual_total >= total_count:
                ...
                rac_status_label = 'Yet To Start'
            elif rac_target == 0:
                rac_status_label = 'In Progress (No videos due yet)'
            elif actual_total >= rac_target:
        ...
        rac_delayed_total = max(rac_target - actual_total - blocked_count - dropped_count, 0)
        if rac_delayed_total > 0:
            rac_delayed_nf    = min(rac_delayed_total, pending_nf)
            rac_delayed_unbox = min(max(rac_delayed_total - rac_delayed_nf, 0), pending_unbox)
        else:
            rac_delayed_nf    = 0
            rac_delayed_unbox = 0

        def fmt_delayed_rac():
            if rac_delayed_nf > 0 and rac_delayed_unbox > 0:
                return f'🔴 Delayed: {rac_delayed_nf} NF | {rac_delayed_unbox} Unboxing'
            elif rac_delayed_nf > 0:
                return f'🔴 Delayed: {rac_delayed_nf} NF'
            elif rac_delayed_unbox > 0:
                return f'🔴 Delayed: {rac_delayed_unbox} Unboxing'
            return ''
        ...
        if rac_status_label in ['Not in RAC Calendar', 'Yet To Start', 'In Progress (No videos due yet)']:
            ...
            if _ds: parts_r.append(_ds)
            feat_status_rac = f'{rac_status_label}\n' + ' | '.join(parts_r)
        elif rac_status_label == 'On Track (Issues Blocking)':
            parts_r = [fmt_completed()]
            _bs = fmt_blocked(); _ds = fmt_dropped()
            if _bs: parts_r.append(_bs)
            if _ds: parts_r.append(_ds)
        ...
        rows.append({
            ...
            'Expected Completion': target_comp,
            'Expected Completion based on RAC Schedule': rac_target if rac_elapsed is not None else '',
            'Actual NF Completion': actual_nf,
            'Actual Unboxing Completion': actual_unbox,
            'Feature Count Status (3/Day rule)': feat_status,
            'Feature Count Status (3/Day rule) as per RAC': feat_status_rac,
    ...
    numeric_cols_r14 = [
        ...
        'Expected Completion', 'Expected Completion based on RAC Schedule',
        'Actual NF Completion', 'Actual Unboxing Completion',
    ...
    for c in df_r14_out.columns:
        if c in numeric_cols_r14:
            total_r14[c] = pd.to_numeric(df_r14_out[c], errors='coerce').fillna(0).sum()
        elif c not in ['Pillar', 'CDL']: total_r14[c] = ''
    df_r14_out = pd.concat([df_r14_out, pd.DataFrame([total_r14])], ignore_index=True)

    # ══════════════════════════════════════════════════════════════════
    # RAC COLUMNS — merged into the output table:
    #   'Exact Number of Days allotted for NF Analysis and Content Development'
    #   'Last date as per RAC calendar'  (last date with a 26C NF cell)
    # ══════════════════════════════════════════════════════════════════
    ...
    def style_r14(row):
        ...
        if 'Feature Count Status (3/Day rule) as per RAC' in cols:
    ...
    def generate_r14_excel(df):
        ...
        for r_idx, row in enumerate(df.itertuples(index=False), 3):
            ...
            for c_idx, val in enumerate(row, 1):
                ...
                if is_total:
                    ...
                    elif first_line in ['Recording Date Not Set', 'Not in RAC Calendar']:
        ...
        col_widths = {
            ...
            'Feature Count Status (3/Day rule) as per RAC': 30,
            'Expected Completion based on RAC Schedule': 14,
